...
function dbUnavailablePage(string $title, string $help, string $msg): never
{
    ...
    exit('<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . htmlspecialchars($title) . '</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-gray-50 min-h-screen flex items-center justify-center p-6"><div class="max-w-md w-full bg-white rounded-2xl shadow p-8 border border-gray-100 text-center"><div class="w-14 h-14 bg-amber-100 text-amber-600 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl font-black">!</div><h1 class="text-xl font-black text-slate-900 mb-2">' . htmlspecialchars($title) . '</h1><p class="text-sm text-gray-600 mb-4">' . $help . '</p><p class="text-xs text-gray-400 font-mono bg-gray-50 rounded-lg p-3 break-all text-left">' . htmlspecialchars($msg) . '</p>' . $installLink . '<a href="index.php" class="inline-block mt-6 text-blue-600 text-sm font-bold hover:underline">Back to Home</a></div></body></html>');
}
