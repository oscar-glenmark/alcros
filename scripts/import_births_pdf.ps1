# Import birth certificate forms from assets/print/forms/official/births.pdf
...
$pdfPath = Join-Path $officialDir 'births.pdf'
$bondWidth = 1700
...
if (-not (Test-Path $pdfPath)) {
    throw "Missing births.pdf at $pdfPath"
}

$tmpPrefix = Join-Path $officialDir 'births-page'
...
foreach ($side in @('front', 'back')) {
    ...
    $rendered = Join-Path $officialDir "births-page-$page.png"
    $dest = Join-Path $formsDir "birth-$side.png"
    ...
    Write-Output "Updated $dest from births.pdf page $page (${bondWidth}px)"
}
