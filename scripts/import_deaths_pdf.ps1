# Import death certificate forms from assets/print/forms/official/deaths.pdf
...
$pdfPath = Join-Path $officialDir 'deaths.pdf'
$bondWidth = 1700
...
if (-not (Test-Path $pdfPath)) {
    throw "Missing deaths.pdf at $pdfPath"
}

$tmpPrefix = Join-Path $officialDir 'deaths-page'
...
foreach ($side in @('front', 'back')) {
    ...
    $rendered = Join-Path $officialDir "deaths-page-$page.png"
    $dest = Join-Path $formsDir "death-$side.png"
    ...
    Write-Output "Updated $dest from deaths.pdf page $page (${bondWidth}px)"
}
