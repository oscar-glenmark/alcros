...
foreach ($type in $sources.Keys) {
    ...
    if (-not (Test-Path $pdfPath) -and $type -eq 'birth') {
        $pdfName = 'Certificate-of-Live-Birth.pdf'
        $pdfPath = Join-Path $officialDir $pdfName
    }
    if (-not (Test-Path $pdfPath) -and $type -eq 'death') {
        $pdfName = 'Certificate-of-Death.pdf'
        $pdfPath = Join-Path $officialDir $pdfName
    }
    if (-not (Test-Path $pdfPath) -and $type -eq 'marriage') {
        $pdfName = 'Certificate-of-Marriage.pdf'
        $pdfPath = Join-Path $officialDir $pdfName
    }
